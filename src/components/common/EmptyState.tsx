...
export const EmptyState: React.FC<EmptyStateProps> = ({ type, onCtaClick }) => {
  const configs = {
    home_no_saves: {
      icon: <Bookmark className="w-12 h-12 text-gold" />,
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-6 text-center animate-fadeIn">
      <div className="w-20 h-20 rounded-2xl bg-panel border border-[#F0B31C]/30 flex items-center justify-center mb-4 shadow-card glow-iqoo">
        {config.icon}
      </div>
      <h3 className="text-lg font-display font-bold uppercase tracking-wider text-gold mb-1.5">{config.heading}</h3>
      <p className="text-xs text-muted max-w-[260px] leading-relaxed mb-5">{config.subtext}</p>
      {config.cta && (
        <button
          onClick={onCtaClick}
