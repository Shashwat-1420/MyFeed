    ...
    <div className="flex-1 flex flex-col p-4 bg-canvas text-ink animate-fadeIn">
      ...
      <form onSubmit={handleSearchSubmit} className="flex items-center space-x-2 mb-4 pt-1">
        ...
        <div className="flex-1 relative flex items-center">
          <Search className="w-4 h-4 text-gold absolute left-3.5" />
          <input
            type="text"
            value={inputVal}
            onChange={(e) => {
            ...
            autoFocus
            className="w-full h-11 bg-panel border border-[#F0B31C]/30 focus:border-[#F0B31C] focus:shadow-glow rounded-xl pl-10 pr-9 text-xs text-ink outline-none transition-all"
          />
          {inputVal && (
            <button
              type="button"
              ...
              className="absolute right-3 text-muted hover:text-gold"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      {isSearchingSemantic && (
        <div className="flex items-center space-x-2 text-xs text-gold mb-3 px-1 animate-pulse font-mono">
      ...
      {!q ? (
        ...
        <div className="space-y-6 flex-1">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-[11px] text-dim hover:text-gold"
                >
                  Clear all
                </button>
              </div>
              <div className="space-y-1.5">
                {recentSearches.map((term, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-panel text-xs text-muted hover:text-gold border border-transparent hover:border-[#F0B31C]/30 transition-all"
                  >
          ...
          {/* Suggested Topics */}
          <div>
            <span className="text-[11px] font-display font-bold uppercase tracking-widest text-muted block mb-2.5">
              Suggested Topics
            </span>
              ...
              {['machine learning', 'figma', 'cs50', 'system design', 'react native', 'flutter', 'career'].map(
                (topic, idx) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-full bg-panel border border-[#F0B31C]/30 text-xs text-muted font-mono hover:text-gold hover:border-[#F0B31C] hover:shadow-glow transition-all"
                  >
                    #{topic}
                  </button>
                )
