  ...
  return (
    <div className="flex-1 flex flex-col p-4 bg-[#0D0D0D] text-[#F2F2F2] animate-fadeIn">
      ...
      <div className="mb-4 pt-1">
        <h1 className="text-xl font-bold text-[#F2F2F2]">Categories</h1>
        ...
        {CATEGORY_LIST.map((cat) => {
          ...
          return (
            <div
              ...
              className={`border-l-4 border-y border-r border-[#2E2E2E] rounded-card p-3.5 flex flex-col justify-between h-[110px] cursor-pointer transition-all duration-200 hover:scale-[1.02] active:scale-95 shadow-card group ${
                isEmpty ? 'opacity-50 hover:opacity-75' : 'hover:border-[#7C6EF6]/40'
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="text-2xl group-hover:scale-110 transition-transform">{cat.emoji}</span>
                <ChevronRight className="w-4 h-4 text-[#5A5A5A] group-hover:text-white transition-colors" />
              </div>

              <div>
                <h3 className="text-xs font-bold text-[#F2F2F2] truncate">{cat.label}</h3>
